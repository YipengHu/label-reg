...
class BaseNet:
    def __init__(self, minibatch_size, image_size):
...
class LocalNet(BaseNet):
    ...
    def __init__(self, minibatch_size, image_size, ddf_levels):
        BaseNet.__init__(self, minibatch_size, image_size)
        self.ddf_levels = ddf_levels
        # defaults
        self.num_channel_initial = 32
        self.initial_bias_local = 0.0
        self.initial_std_local = 0.0
        self.initialiser_local = tf.random_normal_initializer(0, self.initial_std_local)
    ...
    def local_net(
            input_image_pair):  # for debug: input_image_pair=tf.concat([tftools.resize_volume(movingImage0, size_target), targetImage0], axis=4)
        # down-sampling
        ...
        # up-sampling
        h3_c = tf.nn.relu(
            tf.contrib.layers.batch_norm(tf.nn.conv3d_transpose(h4, W3_c, h3c.get_shape(), strides_down, "SAME")))
        ...
        h2_c = tf.nn.relu(
            tf.contrib.layers.batch_norm(tf.nn.conv3d_transpose(h3_, W2_c, h2c.get_shape(), strides_down, "SAME")))
        ...
        h1_c = tf.nn.relu(
            tf.contrib.layers.batch_norm(tf.nn.conv3d_transpose(h2_, W1_c, h1c.get_shape(), strides_down, "SAME")))
        ...
        h0_c = tf.nn.relu(
            tf.contrib.layers.batch_norm(tf.nn.conv3d_transpose(h1_, W0_c, h0c.get_shape(), strides_down, "SAME")))
...
self.transform_initial = helper.identity_transform_vector()
